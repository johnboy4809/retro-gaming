...
<head>
    ...
    <title>Edit Platform - Admin Backend</title>
...
<body class="dashboard-body font-sans antialiased">
    ...
    <div class="w-full max-w-4xl mx-auto px-4 sm:px-8 py-6">
        ...
        <div class="glass-card rounded-2xl border border-retro-border border-opacity-50 overflow-hidden shadow-2xl p-8">
            <h2 class="font-arcade text-2xl font-bold text-white mb-6 border-b border-retro-border pb-4">Edit Sub-Platform: {{ $subPlatform->name }}</h2>
            
            <form action="{{ route('sub-platforms.update', $subPlatform->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="platform_id" required class="w-full px-4 py-3 bg-retro-bg rounded-lg border border-retro-border focus:border-retro-cyan text-white text-sm">
                            ...
                            @foreach($platforms as $platform)
                                <option value="{{ $platform->id }}" {{ old('platform_id', $subPlatform->platform_id) == $platform->id ? 'selected' : '' }}>{{ $platform->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-tech text-gray-400 uppercase tracking-wider mb-2">Sub-Platform Name</label>
                        <input type="text" name="name" value="{{ old('name', $subPlatform->name) }}" required class="w-full px-4 py-3 bg-retro-bg rounded-lg border border-retro-border focus:border-retro-cyan text-white text-sm" placeholder="e.g. MAME">
                    </div>
                    <div>
                        <label class="block text-xs font-tech text-gray-400 uppercase tracking-wider mb-2">Slug (URL Parameter)</label>
                        <input type="text" name="slug" value="{{ old('slug', $subPlatform->slug) }}" required class="w-full px-4 py-3 bg-retro-bg rounded-lg border border-retro-border focus:border-retro-cyan text-white text-sm" placeholder="e.g. mame">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="order_index" value="{{ old('order_index', $subPlatform->order_index) }}" required class="w-full px-4 py-3 bg-retro-bg rounded-lg border border-retro-border focus:border-retro-cyan text-white text-sm">
                    </div>
                    <div class="flex items-center h-full pt-6">
                        <label class="flex items-center space-x-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $subPlatform->is_active) ? 'checked' : '' }} class="w-5 h-5 text-retro-cyan bg-retro-bg border-retro-border rounded focus:ring-retro-cyan focus:ring-2">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            fetch('{{ route('admin.screenscraper.systems') }}')
                ...
                .then(systems => {
                    ...
                    // Select existing DB value or old form value
                    const currentValue = "{{ old('screenscraper_id', $subPlatform->screenscraper_id) }}";
